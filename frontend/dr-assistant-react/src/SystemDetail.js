...
function SystemDetail({ system, user, onApprove, onUpdate, allSystems }) {
  ...
  useEffect(() => {
    if (alertSearchTerm) {
      // Simulate alert search - replace with actual API call
      const mockAlerts = [
        { id: "alert1", name: "High CPU Usage" },
        { id: "alert2", name: "Memory Leak Detected" },
        { id: "alert3", name: "Database Connection Failed" }
      ].filter(a => a.name.toLowerCase().includes(alertSearchTerm.toLowerCase()));
  ...
  try {
    ...
    // Get current system names to detect new externals
    const currentSystemNames = allSystems?.map(sys => sys.name) || [];
    
    // Prepare payload with force_external=true
    const payload = {
      dr_data: editedData.dr_data,
      system_type: editedData.system_type,
      source_reference: editedData.source_reference || null,
      ...
      key_contacts: editedData.key_contacts
        ...
        .map(d => d.trim())
        .filter(Boolean),
      force_external: true  // Always force external creation
    };
    ...
    // Find any newly created external systems
    const allDeps = [
      ...editedData.upstream_dependencies.map(tag => tag.text),
      ...editedData.downstream_dependencies.map(tag => tag.text)
    ];
    
    const newExternalSystems = allDeps.filter(dep => 
      !currentSystemNames.includes(dep)
    );

    if (newExternalSystems.length > 0) {
      alert(`The following external systems were automatically created:\n${newExternalSystems.join('\n')}`);
    }
  ...
  if (editMode) {
    return (
      <div className={`detail-panel edit-mode`}>
        <div className="system-header">
          <h3>{system.name}</h3>
          {showSourceBadge && (
            <span className="system-source-badge">
        ...
        <div className="form-group">
          ...
          <DependencyInput
            tags={editedData.upstream_dependencies}
            setTags={(newTags) => setEditedData({...editedData, upstream_dependencies: newTags})}
            suggestions={[...dependencySuggestions, ...filteredAlerts.map(a => ({
              id: a.id,
              text: a.name
            }))]}
          />
